...
- `docker compose up -d` - Run the application with Docker (includes PostgreSQL 17, Redis, Omnisearch app, and SearXNG)
- `docker compose down` - Stop all containers
- `docker compose down -v` - Stop all containers and remove volumes (deletes database data)
- `docker pull ghcr.io/miurla/omnisearch:latest` - Pull prebuilt Docker image
...
- Omnisearch Cloud deployments block `ENABLE_AUTH=false` automatically
